...
class Reviewer(object):
    ...
    * Track reviewed revisions in history_path.
    ...
    def get_patch(self, change):
        ...
        args = "cd /home/timothy/git/lustre-release ; git fetch https://" + GERRIT_HOST + "/" + GERRIT_PROJECT + " " + change['revisions'][revision]['ref'] + "&& git format-patch -1 -n --stdout FETCH_HEAD | head -n -3"
    ...
    def build_patch(self, change):
        revision = change.get('current_revision')
        args = "cd /home/timothy/git/lustre-release ; git fetch https://" + GERRIT_HOST + "/" + GERRIT_PROJECT + " " + change['revisions'][revision]['ref'] + "&& git checkout FETCH_HEAD"
        pipe = subprocess.Popen(args, shell=True, stdin=subprocess.PIPE,
                                stdout=subprocess.PIPE,
                                stderr=subprocess.PIPE)
        out, err = pipe.communicate("")
...
"""
    ...
    for f in files:
        ...
        patch_revision = os.getxattr(path, "user.patch_revision").decode(errors="replace")
