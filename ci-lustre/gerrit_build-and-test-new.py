...
TESTS = [
    {
        "Command": "cd /home/timothy/git/lustre-release ; ~/ws/ktest/ci-lustre/checkpatch-parallel",
...
"""
    ...
    def run_tests(self, branchwide, change_id, home_path):
        rows = ""
        for test in TESTS:
            if branchwide and test["Type"] == TestType.PR:
                continue

            if not branchwide and test["Type"] == TestType.REPORT:
                continue
...
"""
        ...
        rows = rows + self.run_tests(change.get("branchwide", False), change_id, home_path)
    ...
    def check_for_branches(self):
        ...
        for branch in BRANCHES:
            change = make_change_from_hash(branch["Branch"], branch["Subject"], branch["Branch"])
            self.review_change(change)
