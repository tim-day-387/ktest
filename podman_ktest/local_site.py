...
DATA_FILES = ["metadata_store.json", "status.txt", "version.json"]


def generate_local_site(ktest_dir, results_dir):
    """Build the local status site from a results directory.

    results_dir must contain the files nginx would normally serve
    alongside the site: metadata_store.json and the per-test .log
    files, plus optionally status.txt and version.json.
    """
    data_dir = Path(results_dir)
    site_dir = Path(ktest_dir) / "ci-lustre" / "static-site"
    out_dir = Path(ktest_dir) / "local-site"

    if not (data_dir / "metadata_store.json").is_file():
        print(f"Warning: {data_dir}/metadata_store.json not found, skipping local site")
        return
